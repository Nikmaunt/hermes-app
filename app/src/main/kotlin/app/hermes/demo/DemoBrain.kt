 ...
 *
 * Everything here is deterministic given its inputs: no clock, no randomness. "Now" is
 * supplied by the caller as [BrainContext] (D22) and the notes are pre-resolved against
 * that same instant (D39).
 */
object DemoBrain {
    ...
    fun brainContext(nowMillis: Long): BrainContext = BrainContext(
        nowMillis = nowMillis,
    ...
    fun provider(notes: List<DemoNote>): FakeProvider {
        val byText: Map<String, String> = notes.associate { it.text to it.rawModelJson }
        return FakeProvider { request, _ ->
            BrainResult.Ok(Completion(byText[request.lastUserText()] ?: EMPTY_REPLY, USAGE))
        }
    }

    fun service(notes: List<DemoNote>): ExtractionService = ExtractionService(provider(notes))
